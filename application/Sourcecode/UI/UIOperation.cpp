...
UIOperation::UIOperation()
{
	...
	for (uint32_t i = 0; i < operationUI_.size(); i++) {
		...
		operationUI_[i]->SetTex_Size({ 128,64 });

		Vector2 pos = {225.f + i * 200.f,670};
		operationUI_[i]->SetPos(pos);
		operationUI_[i]->SetScale(Vector2(0.3f, 0.3f));
		...
		pos = { operationUI_[i]->GetPos().x + 100.f,675.f};
	...
	operationUI_[2]->SetTex_LeftTop({ 384,160 });
